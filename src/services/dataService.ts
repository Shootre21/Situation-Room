...
async function fetchNoaaTides(): Promise<MaritimeData[]> {
  const stationRefs = [
  ...
  const responses = await Promise.all(stationRefs.map(async station => {
...
export async function fetchMaritime(): Promise<MaritimeData[]> {
  try {
    ...
    const tidesPromise = fetchNoaaTides().catch(() => [] as MaritimeData[]);
  ...
  try {
    ...
    const noaaAlerts = await fetch('https://api.weather.gov/alerts/active?status=actual&message_type=alert')
      .then(async res => {
        ...
        const data = await res.json();
        return (data.features ?? []).slice(0, 40).map((f: any, idx: number) => ({
          id: `noaa-${idx}-${f.id}`,
        ...
        } as WeatherData)).filter((w: WeatherData) => isValidCoordinate(w.lat, w.lng));
      })
